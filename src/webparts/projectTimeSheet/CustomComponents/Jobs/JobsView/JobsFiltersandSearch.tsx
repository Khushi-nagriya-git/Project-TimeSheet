...
const JobsFiltersandSearch:React.FC<JobsHeaderProps> = ({ projectsData ,jobsData, onJobFilterChange,onJobStatusChange,setSearchQuery ,searchQuery,setFilteredJobs,onJobsAssigneesChange}) => {
  ...
  useEffect(() => {
    ...
    const uniqueAssignees = flattenedAssignees.filter(
      (assignee: { id: any; }, index: any, self: { findIndex: (arg0: (a: any) => boolean) => any; }) =>
      ...
      <Grid container spacing={1} alignItems="center">
        ...
        <Grid item>
          ...
          <Dropdown
            ...
            multiSelect
            options={assignees.map((assignees: any) => ({
              key: assignees.id,
              text: assignees.name,
              selected: selectedAssignees.indexOf(assignees.name) > -1,
              checkbox: true,
            }))}
            styles={{
              root: {
