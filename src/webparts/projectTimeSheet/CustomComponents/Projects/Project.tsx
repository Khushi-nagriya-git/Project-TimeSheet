...
import {React, useState, useEffect,MenuIcon, FormComponent,CircularProgress, useEmployeeTimeSheetContext ,SideNavigation,Drawer,getDepartments, initialState, CustomFormData, ProjectStyle, DeleteDialogBoxProps, IProjectProps, addProjects, getProjectListData, deleteProject, updateUserRecords,  getJobListData, ProjectsData, projectsInitialState, styled, Button, Grid, ProjectHeader, ProjectTable, Alert, DepartmentView, MyTeam, Box,} from "../../../../index";
import { IconButton } from "@mui/material";

const DrawerContainer = styled(Drawer)({ width: 500, flexShrink: 0});
...
const Project: React.FC<IProjectProps> = (props: IProjectProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await getDepartments(props.absoluteURL, props.spHttpClient, setDepartmentNames, "projects" );
  ...
  useEffect(() => {
    ...
    if (  deleteSuccessfullyAlert ||  addSuccessFullyAlert ||  editSuccessFullyAlert ) {
      timer = setTimeout(() => {  setAlert(false);  setDeleteSuccessfullyAlert(false);  setAddSuccessFullyAlert(false);  setEditSuccessFullyAlert(false);}, 5000);
    }
  ...
  const handleDelete = async () => {
    await deleteProject( props.absoluteURL, props.spHttpClient, deletedProjectId, setProjectsData );
    await getProjectListData( props.absoluteURL, props.spHttpClient, setProjectsData, props.loggedInUserDetails, props.isUserAdmin );
    setIsOpen(false);
    setAlert(true);
  ...
  return (
    <div>
      <ProjectStyle.FullHeightGrid container>
        <ProjectStyle.FullHeightGrid  item  style={{ display: "flex", height: "100%", width: "100%" }} >
          <ProjectStyle.MainContainer>
            <ProjectStyle.Content>
              ...
              <ProjectStyle.NavigationLinks>
                ...
                <ProjectStyle.NavLink className={myDataActiveLink === "Employee" ? "active" : ""} onClick={() => handleTabChange("Employee")} >  Projects </ProjectStyle.NavLink>
                <ProjectStyle.NavLink className={ myDataActiveLink === "ProjectMembers" ? "active" : "" } onClick={() => handleTabChange("ProjectMembers")}> Project Members </ProjectStyle.NavLink>
                {props.isUserAdmin && ( <ProjectStyle.NavLink  className={  myDataActiveLink === "Department" ? "active" : "" } onClick={() => handleTabChange("Department")} >  Departments </ProjectStyle.NavLink> )}

                <Grid item sx={{ marginLeft: "auto", marginBottom: "10px" }}>
                  {(props.isUserReportingManager || props.isUserAdmin) && myDataActiveLink === "Employee" && (
                      <Button variant="contained" onClick={handleAddProject}  sx={{  backgroundColor: "#023E8A",  borderRadius: "5px",  width: "120px",  height: "35px",  fontSize: "13px",  textTransform: "none",  }} > Add Project</Button>
                    )}
                </Grid>
      ...
      {(isOpen || deleteAlert) && (
        <DeleteDialogBoxProps open={isOpen || deleteAlert} handleClose={function (): void { setIsOpen(false); setDeleteAlert(false); }} isJobAvailable={isJobAvailable} handleDeleteAction={handleDelete} />
      )}

      {alert && (
        <Alert severity={deleteSuccessfullyAlert ? "warning" : "success"} onClose={function (): void { setDeleteSuccessfullyAlert(false); setEditSuccessFullyAlert(false); setAddSuccessFullyAlert(false); }} sx={{ position: "fixed", top: "50px", right: "20px", zIndex: 9999, }} > {deleteSuccessfullyAlert ? "Project has been successfully deleted!" : addSuccessFullyAlert ? "Project has been successfully added!" : editSuccessFullyAlert ? "Project has been successfully updated!" : ""}</Alert>
      )}

      {addFormOpen && (
        <FormComponent mode={mode} initialData={currentData ?? undefined} onSubmit={handleSubmit} spHttpClient={props.spHttpClient} absoluteURL={props.absoluteURL} context={props.context} open={addFormOpen} setAddFormOpen={setAddFormOpen} peoplePickerDefaultManager={peoplePickerDefaultManager} peoplePickerDefaultReportingManager={ peoplePickerDefaultReportingManager } peoplePickerDefaultTeam={peoplePickerDefaultTeam} departmentNames={departmentNames} />
      )}
    </div>
  );
};
