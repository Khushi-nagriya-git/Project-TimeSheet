...
const DepartmentView = (props: {
  ...
  useEffect(() => {
    ...
    if (props.selectedDepartmentName.length > 0) {
      filteredProjects = filteredProjects.filter((project: { DepartmentsORTeam: string }) =>
    ...
    if (props.searchQuery.trim() !== '') {
      filteredProjects = filteredProjects.filter((project: any) => {
        return (
          ...
          project.DepartmentsORTeam.toLowerCase().includes(props.searchQuery.toLowerCase())
        );
      });
    }
  ...
  return (
    <Box sx={{ height: "285px", marginTop: "-15px", overflow: "auto" }}>
    <TableContainer>
