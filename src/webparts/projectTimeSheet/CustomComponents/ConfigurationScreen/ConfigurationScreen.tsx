...
const ConfigurationScreen: React.FC<IConfigurationScreenProps> = (
  ...
  return (
    <React.Fragment>
      <Content>
        <Box
          sx={{ display: "flex", height: "auto", backgroundColor: "#f8f8f9" }} >
          {/* Left Header: Takes up around 25-30% of the screen width */}
          <Box sx={{  width: "20%",  backgroundColor: "#fff", boxShadow: "2px 0px 5px rgba(0, 0, 0, 0.1)", }}>
            <ConfigurationScreenLeftNavigation setTab={setTab} />
          </Box>

          {/* Right Content: Takes up the remaining 70-75% of the screen */}
          <Box sx={{ width: "80%", display: "flex", flexDirection: "column", padding: "10px", }}>
            {/* Top Header */}
            <Box sx={{  marginBottom: "20px", backgroundColor: "#fff", padding: "10px", borderRadius: "10px", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)", }} >
              <TopNavigation heading={tab ==="General Settings" ? "Configuration" : "Add Departments"} />
            </Box>

            <Box sx={{ marginBottom: "20px", backgroundColor: "#fff", padding: "10px", borderRadius: "10px", boxShadow: "0px 2px 5px rgba(0, 0, 0, 0.1)", }} >
              {tab === "General Settings" && (
                <GeneralSettings absoluteURL={props.absoluteURL} setReload = {props.setReload} configurationListData={props.configurationListData} spHttpClient={props.spHttpClient} context={props.context} configurationListDataLength ={props.configurationListDataLength} title={props.title} ></GeneralSettings>
              )}
